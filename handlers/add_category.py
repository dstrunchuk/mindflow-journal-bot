...
@router.message(Command("добавитькатегорию"))
async def cmd_add_category(message: Message, database, categorizer):
    """Обработчик команды /добавитькатегорию - добавление пользовательской категории"""
    try:
        user_id = message.from_user.id
        text = message.text.strip()
        
        # Извлекаем параметры команды
        if text.startswith('/добавитькатегорию'):
            params = text[18:].strip()  # Убираем '/добавитькатегорию ' из начала
            
        if not params:
            await message.answer("""🔧 <b>Добавление категории</b>

Использование: /добавитькатегорию Название:ключ1,ключ2,ключ3

Примеры:
• /добавитькатегорию Работа:проект,задача,дедлайн
• /добавитькатегорию Здоровье:спорт,диета,врач
• /добавитькатегорию Финансы:деньги,бюджет,траты""", parse_mode="HTML")
            return
